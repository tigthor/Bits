...
Below are some notes on how to build GameCredits Core for Windows.

The options known to work for building GameCredits Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the GameCredits Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the GameCredits Core applications.
...
Note that for WSL the GameCredits Core source path MUST be somewhere in the default mount file system, for
example /usr/src/gamecredits, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\gamecredits`, for example:

    make install DESTDIR=/mnt/c/workspace/gamecredits
...
causes two of the gamecredits executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the gamecredits code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the gamecredits source code).
