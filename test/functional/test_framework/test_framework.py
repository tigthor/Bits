...
class BitcoinTestFramework(object):
    ...
    def main(self):
        ...
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop gamecreditsds after the test execution")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: gamecreditsds were not stopped and may still be running")
    ...
    def start_node(self, i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr=None):
        ...
        if binary is None:
            binary = os.getenv("gamecreditsD", "gamecreditsd")
        args = [binary, "-datadir=" + datadir, "-server", "-keypool=1", "-discover=0", "-rest", "-logtimemicros", "-debug", "-debugexclude=libevent", "-debugexclude=leveldb", "-mocktime=" + str(self.mocktime), "-uacomment=testnode%d" % i]
        if extra_args is not None:
            args.extend(extra_args)
        self.bitcoind_processes[i] = subprocess.Popen(args, stderr=stderr)
        self.log.debug("initialize_chain: gamecreditsd started, waiting for RPC to come up")
    ...
    def assert_start_raises_init_error(self, i, dirname, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'gamecreditsd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "gamecreditsd should have exited with an error"
                else:
                    assert_msg = "gamecreditsd should have exited with expected error " + expected_msg
    ...
    def _initialize_chain(self, test_dir, num_nodes, cachedir):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("gamecreditsD", "gamecreditsd"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
    ...
    def _wait_for_bitcoind_start(self, process, datadir, i, rpchost=None):
        ...
        while True:
            if process.poll() is not None:
                raise Exception('gamecreditsd exited with status %i during initialization' % process.returncode)
            try:
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("gamecreditsD", "gamecreditsd"),
                          help="gamecreditsd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("gamecreditsD", "gamecreditsd"),
                          help="gamecreditsd binary to use for reference nodes (if any)")
